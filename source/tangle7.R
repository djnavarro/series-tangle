...
points <- use_seed(4) %>% scene_grid(xpos = 1:5, ypos = 1:5, size = .8)
...
pic <- ggplot(dat, aes(x, y, colour = shade)) + 
  ...
  scale_color_gradientn(colours = sample(colours(), 4)) +
  theme_void() + 
  coord_equal() + 
  theme(plot.background = element_rect(fill = bg, colour = bg))

ggsave(
  filename = here::here("image", "tangle7.png"),
  bg = bg,
  plot = pic,
  width = 5000/300,
  height = 5000/300,
